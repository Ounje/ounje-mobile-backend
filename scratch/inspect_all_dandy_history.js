...
async function run() {
  try {
    ...
    const entries = await LedgerEntry.find({ accountId: new mongoose.Types.ObjectId("69ebe217af62f56bd93b5377") }).sort({ createdAt: 1 }).limit(40).lean();
    console.log(`Found first ${entries.length} entries for Dandy (chronological):`);
