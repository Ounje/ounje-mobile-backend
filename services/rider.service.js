...
const { generateRiderId } = require("../utils/generateProfileId");
...
const completeRiderRegistration = async (userId, data, files) => {
	...
	if (riderProfile.guarantor && riderProfile.guarantor.name) {
		throw new Error("Registration already completed. Guarantor already exists.");
	}

	if (!modeOfDelivery || !["Bicycle", "Motorcycle"].includes(modeOfDelivery)) {
		throw new Error("Invalid mode of delivery. Must be 'Bicycle' or 'Motorcycle'");
	}
	...
	if (modeOfDelivery === "Motorcycle") {
		if (!files.driversLicense || !files.driversLicense[0]) {
			throw new Error("Driver's license document is required for Motorcycle riders");
		}
	...
	// Generate a unique rider ID if one hasn't been assigned yet
	if (!riderProfile.riderId) {
		riderProfile.riderId = await generateRiderId();
	}
	...
	return {
		...
		profileId: riderProfile.riderId,   // human-readable ID shown on the app
		userId: user._id,
		name: user.name,
		phone: user.phone,
		role: user.role,
...
const getRiderProfile = async (userId) => {
    // Step A: Added "riderId" at the start of the select projection fields string
    const riderProfile = await RiderProfile.findOne({ user: userId })
        .populate("user", "name phone email role")
        .select(
            "riderId modeOfDelivery guarantor driversLicense nin status operatingArea isActive currentLocation earnings ratings averageRating ratingCount totalDeliveries rankingScore tier profilePicture setupComplete bankDetails",
        );
    ...
    // Step B: Added riderId (the database _id) and profileId (human readable ID) to the returned object
    return {
        riderId: riderProfile._id,
        profileId: riderProfile.riderId,      // human-readable ID shown on the app
