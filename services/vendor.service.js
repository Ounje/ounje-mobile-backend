...
class VendorService {
    ...
    async getNearbyVendors({ lat, lng, userId }) {
        ...
        try {
            ...
            return { status: "success", source: "default-fallback", results: allVendors.length, data: allVendors };
        } catch (error) {
            throw error;
        }
    }
    ...
    async getVendorWithProducts(vendorId) {
        const vendor = await Vendor.findById(vendorId).populate("menu").populate("foodItems");
        if (!vendor) throw new Error("Vendor not found");
        return vendor;
    }
    ...
    async completeRegistration(vendorId, data, fileUrl) {
        ...
        const storeDetailsData = this._buildStoreDetails(data, fileUrl, statusResult);
        ...
        // 7. Format Response
        return this._formatRegistrationResponse(vendor, storeDetailsData, statusResult);
    }
    ...
    _validateBasicRegistrationData(data, fileUrl) {
        ...
        if (!storeName || !storeType || !servicesOffered) {
            throw new Error("Store name, store type and services offered are required");
        }
        if (!["physicalStore", "onlineStore"].includes(storeType)) {
            throw new Error("Invalid store type. Must be 'physicalStore' or 'onlineStore'");
        }
        if (!["InstantMeals", "preOrderMeals", "hybridMeals"].includes(servicesOffered)) {
            throw new Error("Invalid services offered. Must be 'InstantMeals', 'preOrderMeals', or 'hybridMeals'");
        }
        if (!fileUrl) {
    ...
    _determineAccountStatus(data) {
        const isVerifiedBusiness = data.isVerifiedBusiness === true || data.isVerifiedBusiness === "true";
        ...
        if (!isVerifiedBusiness) {
            if (!needCACHelp) {
                return {
                    ...
                    response: {
                        success: false,
                        message: "Your business needs to be registered. Would you like us to help you with CAC registration?",
                        needsCAC: true,
                        status: 400
                    }
                };
            ...
            if (needsCACSupport) {
                warningMessage = "Your account is pending. Our support team will contact you regarding CAC registration assistance.";
            } else {
                warningMessage = "Please do well to complete your CAC registration so that your business will be safe from legal fines.";
            }
            return { shouldReturnError: false, status, needsCACSupport, warningMessage, isVerifiedBusiness };
        }
        ...
        return { shouldReturnError: false, status: "active", needsCACSupport: false, warningMessage: null, isVerifiedBusiness };
    }
    ...
    _parsePreorderPeriods(data) {
        ...
        if (data.preorderPeriods && Array.isArray(data.preorderPeriods)) {
            ...
            if (periods.length === 0 && data.orderingTime && data.preparationTime && data.period) {
                periods.push({ orderingTime: data.orderingTime, preparationTime: data.preparationTime, period: data.period });
            }
        }

        if (periods.length === 0) {
            throw new Error("At least one preorder period (orderingTime, preparationTime, and period) is required for pre-order services");
        }

        for (const pp of periods) {
            if (!pp.orderingTime || !pp.preparationTime || !pp.period) {
                throw new Error("Each preorder period must include orderingTime, preparationTime, and period");
            }
            if (!["breakfast", "lunch", "dinner"].includes(pp.period)) {
                throw new Error(`Invalid period: ${pp.period}. Must be one of 'breakfast', 'lunch', or 'dinner'`);
            }
        }
        return periods;
    }
    ...
    _parseTimePeriods(data) {
        ...
        if (data.timePeriod && Array.isArray(data.timePeriod)) {
            ...
            if (periods.length === 0 && data.day && data.openingHour && data.closingHour) {
                periods.push({ day: data.day, openingHour: data.openingHour, closingHour: data.closingHour });
            }
        }

        if (periods.length === 0) {
            throw new Error("At least one time period is required for instant/hybrid meal services");
        }
        ...
        return periods.map(tp => {
            if (!tp.day || !tp.openingHour || !tp.closingHour) {
                throw new Error("Each time period must include day, openingHour, and closingHour");
            }
            const day = tp.day.toLowerCase();
            if (!validDays.includes(day)) {
                throw new Error(`Invalid day: ${tp.day}. Must be one of: ${validDays.join(", ")}`);
            }
            return { ...tp, day };
        });
    }
    ...
    _formatRegistrationResponse(vendor, storeDetailsData, statusResult) {
        ...
        return {
            ...
            message: statusResult.status === "pending"
