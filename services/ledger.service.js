...
const ensureAccount = async (userId, type) => {
  ...
  if (!account) {
    account = await LedgerAccount.create({ userId, type, availableBalance: 0, pendingBalance: 0, holdBalance: 0 });
  }
  return account;
};
 ...
 */
const creditAccount = async (userId, userType, amount, reason, orderId, metadata = {}) => {
 ...
 */
const debitAccount = async (userId, userType, amount, reason, metadata = {}) => {
  if (amount <= 0) throw new Error("Amount must be positive");
  ...
  try {
    ...
    if (account.availableBalance < amount) {
      throw new Error(`Insufficient balance. Available: ${account.availableBalance}, Requested: ${amount}`);
    }
 ...
 */
const reverseReserve = async (userId, userType, amount, reason = "User cancelled") => {
  if (amount <= 0) throw new Error("Amount must be positive");
 ...
 */
const getTransactionHistory = async (userId, userType, limit = 20, skip = 0) => {
  const account = await LedgerAccount.findOne({ userId, userType });
