...
exports.regenerateDeliveryOtp = async (req, res) => {
	try {
		const order = await Order.findById(req.params.id);
		...
		// Only allow if order is picked up or being delivered
		if (![ORDER_STATUS.RIDING, ORDER_STATUS.PICKED_UP].includes(order.status) && order.subStatus !== ORDER_SUB_STATUS.PICKED_UP) {
			// Looser check: as long as it's not delivered or cancelled
		...
		logger.info(`OTP regenerated for order ${order._id} by customer`);
		...
		logger.error(`REGENERATE_OTP_ERROR: ${error.message}`);
		res.status(500).json({ message: "Failed to regenerate OTP", error: error.message });
	}
};
...
exports.completeDelivery = async (req, res) => {
	try {
		...
		logger.error(`COMPLETE_DELIVERY_ERROR: ${error.message}`);
