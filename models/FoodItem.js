		...
		const pipeline = [
			...
			{ $match: { "vendorInfo.isActive": true } },
			{
				$project: {
					...
					originalPrice: "$subCategory.items.originalPrice", // Included in projection
	...
	try {
		...
		for (const subCat of subCategories) {
			...
			for (const item of subCat.items) {
				...
				// Calculate 10% platform markup
				const markedUpPrice = Math.round(inputPrice * 1.10);

				builtItems.push({
					name: item.name,
					originalPrice: inputPrice, // Vendor true price
					price: markedUpPrice,       // Marked-up price customers see
...
const updateFoodItem = async (req, res) => {
	try {
		...
		// Clean up subcategory schemas check: fields validation
		// Note: Because subCategory in FoodItem is now an Array of objects containing sub-items,
		// directly assigning raw array objects from req.body requires re-applying structural pricing constraints.
		if (req.body.subCategory && Array.isArray(req.body.subCategory)) {
			req.body.subCategory.forEach(subCat => {
				if (subCat.items && Array.isArray(subCat.items)) {
					subCat.items.forEach(item => {
						if (item.price !== undefined) {
							const inputPrice = Number(item.price);
							item.originalPrice = inputPrice;
							item.price = Math.round(inputPrice * 1.10);
						}
					});
				}
			});
...
const getAllFoodItems = async (req, res) => {
	try {
		...
		const populate = {
			path: "vendor",
			select:
				"storeDetails name img profileImage bannerUrl logoUrl description averageRating totalOrders location",
		};
			...
			group.items.forEach((selectionItem) => {
				...
				if (foundItem) {
					populatedItems.push({
						...
						name: foundItem.name,
						img: foundItem.img || "",
						originalPrice: foundItem.originalPrice, // Populating the original price field
...
const getAllCombos = async (req, res) => {
	try {
		...
		const populateOptions = [
			...
			{ path: "selections.items.item", select: "name img description price originalPrice" },
		...
		const populateOptions = [
			{
				...
				select: "name img description price originalPrice",
			},
			{ path: "comboGroup", select: "name description" },
		];
